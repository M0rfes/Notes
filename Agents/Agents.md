...
In short, an **Agent** is an [[LLMs|LLM]] with tools. But when does an LLM with a tool go from a chatbot that can look up stuff on the web or in a database and become an agent that can automate almost anything?
